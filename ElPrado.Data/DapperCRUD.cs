...
namespace ElPrado.Data
{
    public static class DapperCRUD
    {
        ...
        public static void SetDialect(Dialect dialect)
        {
            switch (dialect)
            {
                case Dialect.Firebird:
                    _dialect = Dialect.Firebird;
                    _encapsulation = "{0}";
                    _getIdentitySql = string.Format("SELECT LASTVAL() AS id");
                    _getPagedListSql =
                        "SELECT FIRST {RowsPerPage} SKIP (({PageNumber}-1) * {RowsPerPage}) {SelectColumns} FROM {TableName} {WhereClause} ORDER BY {OrderBy}";
                    break;
                ...
                case Dialect.DB2:
                    ...
                    _encapsulation = "\"{0}\"";
                    _getIdentitySql =
                        string.Format("SELECT CAST(IDENTITY_VAL_LOCAL() AS DEC(31,0)) AS \"id\" FROM SYSIBM.SYSDUMMY1");
                    _getPagedListSql =
            ...
            StringBuilderCache(masterSb, $"{typeof(T).FullName}_DeleteWhere{conditions}", sb =>
            {
                ...
                if (!conditions.ToLower().Contains("where"))
                    throw new ArgumentException(
                        "DeleteList<T> requires a where clause and must contain the WHERE keyword");
        ...
        //This allows use of the DataAnnotations property in the model and have the DapperCRUD engine just figure it out without a reference
        ...
        public class ColumnNameResolver : IColumnNameResolver
        {
            public virtual string ResolveColumnName(PropertyInfo propertyInfo)
            {
                ...
                {
                    columnName = propertyInfo.Name.ToUnderscoreCase().ToUpper();
                }
                else
                {
    ...
    /// Custom for Dapper.DapperCRUD to exclude a property from Select methods
    /// </summary>
    [AttributeUsage(AttributeTargets.Property)]
    ...
    /// Custom for Dapper.DapperCRUD to exclude a property from Insert methods
    /// </summary>
    [AttributeUsage(AttributeTargets.Property)]
    ...
    /// Custom for Dapper.DapperCRUD to exclude a property from Update methods
    /// </summary>
    [AttributeUsage(AttributeTargets.Property)]
